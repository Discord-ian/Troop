...
class Interface(BasicInterface):
    def __init__(self, client, title, language, logging=False):
        ...
        try:
            ...
            try:
                ...
                self.root.tk.call(
                    "wm", "iconphoto", self.root._w, PhotoImage(file=icon + ".gif")
                )
        ...
        self.console = Console(
            ...
            font="Font",
            highlightthickness=0,
            yscrollcommand=self.c_scroll.set,
        )
        ...
        self.text.bind(
            "<Button-2>" if SYSTEM == MAC_OS else "<Button-3>", self.mouse_press_right
        )
        self.text.bind(
            "<Button-2>" if SYSTEM != MAC_OS else "<Button-3>", lambda *e: "break"
        ...
        self.text.tag_configure(
            SEL, background=COLOURS["Background"]
            ...
            if len(loc) > 0:
                ...
                for i in range(0, len(loc), 2):
                    ...
                    if start[0] == end[0]:
                        ...
                        count += (
                            self.convert(self.text.index("{}.end".format(start[0])))[1]
                            - start[1]
                        )
                        ...
                        for line in range(start[0] + 1, end[0]):
                            ...
                            count += self.convert(
                                self.text.index("{}.end".format(line))
                            )[1]
    ...
    def send_set_mark_msg(self):
        ...
        self.add_to_send_queue(
            MSG_SET_MARK(self.text.marker.id, self.text.marker.get_index_num(), reply=0)
        )
        return
    ...
    def apply_operation(self, operation, index_offset=0, **kwargs):
        ...
        if self.user_disabled() is False:
            ...
            self.text.handle_operation(
                MSG_OPERATION(self.text.marker.id, operation, self.text.revision),
                client=True,
            )
    ...
    def move_marker_end(self):
        ...
        index = self.text.tcl_index_to_number(
            self.text.index("@{},{}".format(new_x, y))
        )  # TODO: This goes one char short?
        self.text.marker.move(index)
        return
    ...
    def get_word_left_index(self, index):
        ...
        for i in range(index, 0, -1):
            if text[i - 1] in self.delimeters and text[i] in self.delimeters:
                break
            elif text[i - 1] in (self.delimeters + self.whitespace) and text[i] not in (
                self.delimeters + self.whitespace
            ):
                break
        else:
            i = 0
    ...
    def get_word_right_index(self, index):
        ...
        for i in range(index, len(text) - 1):
            if text[i - 1] in self.delimeters and text[i] in self.delimeters:
                break
            elif text[i - 1] not in (self.delimeters + self.whitespace) and text[i] in (
                self.delimeters + self.whitespace
            ):
                break
        else:
    ...
    def single_line_evaluate(self, event=None):
        ...
        if self.input_blocking:
            ...
            if self.text.get(a, b).lstrip() != "":
                ...
                self.add_to_send_queue(
                    MSG_EVALUATE_BLOCK(self.text.marker.id, row, row)
                )
    ...
    def undo(self, event=None):
        ...
        if len(self.text.undo_stack):
            ...
            self.apply_operation(
                self.new_operation(*op.ops),
                index=get_operation_index(op.ops),
                undo=True,
            )
        return "break"
    ...
    def ApplyColours(self, event=None):
        ...
        # Text & Line numbers
        self.text.config(
            bg=COLOURS["Background"], insertbackground=COLOURS["Background"]
        )
